...
import { fetchEvents, createEvent, updateEvent, deleteEvent, fetchEventAttendees } from '@/store/slices/eventsSlice';
...
const Admin = () => {
  ...
  const { events, attendees, loading } = useAppSelector((state) => state.events);
  ...
  const EventForm = ({ onSubmit, buttonText }: { onSubmit: (e: React.FormEvent) => void; buttonText: string }) => (
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">Manage events and view attendees</p>
          </div>
              ...
              <Card key={event._id} className="shadow-card">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => handleViewAttendees(event)}>
                ...
                <CardContent className="space-y-2">
                  ...
                  <p className="text-sm"><strong>Date:</strong> {formatEventDate(event.date)}</p>
                  <p className="text-sm"><strong>Location:</strong> {event.location}</p>
                  <p className="text-sm"><strong>Capacity:</strong> {event.bookedSeats}/{event.capacity}</p>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <Dialog open={isAttendeesOpen} onOpenChange={setIsAttendeesOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Event Attendees - {selectedEvent?.title}</DialogTitle>
            </DialogHeader>
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {attendees.length === 0 ? (
                ...
                attendees.map((attendee) => (
                  <div key={attendee._id} className="p-3 border border-border rounded-lg">
